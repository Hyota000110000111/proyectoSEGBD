...
# ====================== LISTAR EMPLEADOS (lectura) ======================
@admin_bp.route('/empleados')
@login_required
@roles_required('ADMINISTRADOR')
def listar_empleados():
    empleados = EmpleadoPublic.query.all()   # usa la vista pública (sin password_hash)
...
def nuevo_empleado():
    if request.method == 'POST':
        ...
        # Si la sucursal es 0 o None, se asigna NULL (para AUDITOR)
        if sucursal_id == 0:
            sucursal_id = None
        # Verificar que la sucursal existe (si se proporcionó)
        if sucursal_id:
...
def eliminar_empleado(id):
    ...
    # Solo ADMINISTRADOR puede eliminar, no necesita verificar sucursal
...
def dashboard():
    ...
    top_medicamentos_nombres = [med[0] for med in top_meds]
    top_medicamentos_cantidades = [int(med[1]) for med in top_meds]
